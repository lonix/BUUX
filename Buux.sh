...
case "$osSelected" in
	...
	ibarch4)
		...
		config_General
		config_Add_Pygrub
		if  [[ ! -f ArchVM_v4.zip ]]; then wget https://lonix.me/mirror/archvm/ArchVM_v4.zip; fi
	...
	6|ibarch5)
		...
		if  [[ ! -f ArchVM_v5.zip ]]; then wget https://lonix.me/mirror/archvm/ArchVM_v5.zip; fi
		wget https://raw.githubusercontent.com/lonix/BUUX/master/img/archlinux.png
		unzip ArchVM_v5.zip
		mv "ArchVM/arch.img" "$domain.img"
		cp archlinux.png /boot/config/domains/$domain.png
	...
	7|tretflix13)
		...
		config_General
		config_Add_Pygrub
		if [[ ! -f Tretflix-v1.3_x64-NAS.zip ]]; then wget https://lonix.me/mirror/tretflix/Tretflix-v1.3_x64-NAS.zip; fi
		...
		if ! command -v xxd>/dev/null; then wget https://lonix.me/mirror/xxd.txz && installpkg xxd.txz; fi
